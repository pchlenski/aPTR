...
def _find_candidates(sample: pd.Series, db: RnaDB) -> (pd.DataFrame, list):
    """
    Given a sample (indexed by MD5 hash IDs), find sequences that are worth looking at. Criteria are:
    * In the relevant RnaDB object database
    * Non-zero read count
    * Multiple MD5s in sample can match to the same genome
...
def solve_all(
    ...
    -----
    otu_table_path: str
        Path to a TSV of coverages, indexed by MD5 hash IDs. This can be generated by process_samples.py
    db_path: str
        Path to a pickled RnaDB object. If not specified, will generate a new database from left_adapter and
        right_adapter.
    left_adapter: str
    ...
    true_values: str
        Path to a TSV of true PTRs. Should have the same columns as the OTU table and be indexed on genome ID.
